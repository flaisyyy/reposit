...
final class Sonic {
    ...
    public Sonic(int inputSampleRateHz2, int numChannels2, float speed2, float pitch2, int outputSampleRateHz) {
        ...
        this.inputBuffer = new short[(this.maxRequired * numChannels2)];
        this.outputBufferSize = this.maxRequired;
        this.outputBuffer = new short[(this.maxRequired * numChannels2)];
        this.pitchBufferSize = this.maxRequired;
        this.pitchBuffer = new short[(this.maxRequired * numChannels2)];
    ...
    public void queueEndOfStream() {
        ...
        int expectedOutputSamples = this.numOutputSamples + ((int) ((((((float) remainingSamples) / (this.speed / this.pitch)) + ((float) this.numPitchSamples)) / (this.rate * this.pitch)) + 0.5f));
    ...
    private boolean previousPeriodBetter(int minDiff2, int maxDiff2, boolean preferNewPeriod) {
        if (minDiff2 == 0 || this.prevPeriod == 0) {
            return false;
        }
        if (preferNewPeriod) {
            if (maxDiff2 > minDiff2 * 3 || minDiff2 * 2 <= this.prevMinDiff * 3) {
                return false;
            }
        } else if (minDiff2 <= this.prevMinDiff) {
            return false;
        }
        return true;
    }
    ...
    private short interpolate(short[] in, int inPos, int oldSampleRate, int newSampleRate) {
        short left = in[inPos];
        short right = in[this.numChannels + inPos];
    ...
    private void adjustRate(float rate2, int originalNumOutputSamples) {
    ...
    private int skipPitchPeriod(short[] samples, int position, float speed2, int period) {
        int newSamples;
        if (speed2 >= 2.0f) {
            newSamples = (int) (((float) period) / (speed2 - 1.0f));
        } else {
            newSamples = period;
            this.remainingInputToCopy = (int) ((((float) period) * (2.0f - speed2)) / (speed2 - 1.0f));
        }
    ...
    private int insertPitchPeriod(short[] samples, int position, float speed2, int period) {
        int newSamples;
        if (speed2 < 0.5f) {
            newSamples = (int) ((((float) period) * speed2) / (1.0f - speed2));
        } else {
            newSamples = period;
            this.remainingInputToCopy = (int) ((((float) period) * ((2.0f * speed2) - 1.0f)) / (1.0f - speed2));
        }
            ...
            do {
                if (this.remainingInputToCopy > 0) {
                    ...
                    if (((double) speed2) > 1.0d) {
                        position += skipPitchPeriod(this.inputBuffer, position, speed2, period) + period;
                    } else {
                        position += insertPitchPeriod(this.inputBuffer, position, speed2, period);
                    }
                }
    ...
    private static void overlapAdd(int numSamples, int numChannels2, short[] out, int outPos, short[] rampDown, int rampDownPos, short[] rampUp, int rampUpPos) {
        for (int i = 0; i < numChannels2; i++) {
            int o = (outPos * numChannels2) + i;
            int u = (rampUpPos * numChannels2) + i;
            int d = (rampDownPos * numChannels2) + i;
